...
import { getTemporalKind } from "../../utils/temporalValue";
...
export function TableData() {
  ...
  const columnMetadataByName = useMemo(
    () => new Map(tableStructure?.map((c) => [c.name, c]) ?? []),
    [tableStructure]
  );
  ...
  const allTableColumns = useMemo<ColumnsType<Record<string, unknown>>>(
    () =>
      columns.map((colName) => {
        ...
        const columnMeta = columnMetadataByName.get(colName);
        const columnTooltipLines = buildColumnHeaderTooltipLines(
          columnMeta?.column_type,
          columnMeta?.comment
        );
        const columnTooltipTitle = columnTooltipLines.length ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {columnTooltipLines.map((line) => (
      ...
      >
        <Form form={insertForm} layout="vertical" size="small">
          {tableStructure?.map((col) => {
            ...
            const temporalKind = getTemporalKind(
              col.column_type,
              currentDatabaseType
            );
            const placeholder = isAutoGenerated
              ? "自动生成, 留空即可"
              : col.nullable
                ? "可选 (NULL)"
                : `请输入 ${col.name}`;
            return (
              ...
              >
                {temporalKind ? (
                  ...
                  <SafeInput placeholder={placeholder} />
                )}
              </Form.Item>
            );
          })}
